...
@Settings(settingsFile = "sheep.json")
public class Sheep extends Herbivorous {

    private static final AnimalUnit ANIMAL_UNIT = AnimalSettingsUtil.getAnimalUnit(Sheep.class);
    ...
    public void reproduce(Area area) {
        ...
        for (Set<Animal> animalSet : animals.values()) {
            for (Animal animal : animalSet) {
                if (animal instanceof Sheep && !area.isFull(Sheep.class)) {
                    int chanceToReproduce = ThreadLocalRandom.current().nextInt(0, 100);
                    if (chanceToReproduce < 2) {
                        Animal bornAnimal = AnimalFactory.createAnimal(Sheep.class);
    ...
    public int getNumberOfMovements() {
        return Sheep.getAnimalUnit().speed();
    }
}
